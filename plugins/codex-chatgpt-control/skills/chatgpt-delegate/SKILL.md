...
The plugin loader derives the current Codex workspace automatically. New Chat
threads and new Work tasks use the matching workspace-named ChatGPT Project. A
missing Project must return a creation confirmation
blocker. Set `confirmCreation: true` only after explicit user approval. Use
`project: false` with `confirmGlobal: true` only when the user specifically
...
creation confirmation, or lost Project context ends the current delegation
...
The plugin loader may persist an explicit blanket approval in
...
before submission: the SDK opens the composer-scoped control, reads the root
`Power` slider, enters `Select model` to read the active model radio, and closes
the menu without changing the selection. When applying both axes it verifies
Power first and the model second. Do not conclude that GPT-5.6 Sol or the 5-of-5
state is unavailable from the collapsed label alone.
