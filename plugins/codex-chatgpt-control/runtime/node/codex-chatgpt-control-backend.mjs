...
async function readAttachmentReadiness(page, files, baseline, timeoutMs) {
  ...
  return page.evaluate((args) => {
    ...
    const inputFiles = Array.from(input.files ?? []).map((file) => ({
      ...
      size: file.size,
      signature: `${normalize(file.name)}\0${file.size}`
    }));
...
async function findSingleVisibleLocator(locator, labels, role) {
  ...
  const count = await locatorCount2(locator);
  let visible;
  for (let index = 0; index < count; index += 1) {
...
async function openOrCreateProjectForNewThread(env, input, timeoutMs = 3e4) {
  ...
  try {
    ...
    await page.goto?.(CHATGPT_HOME2, { waitUntil: "domcontentloaded", timeout: timeoutMs });
    await page.waitForTimeout?.(500);
...
async function resolveProjectPageUrl(page, row, timeoutMs) {
  ...
  if (await locatorCount3(projectLinks) > 0) {
...
async function createProject(page, project, timeoutMs) {
  ...
  if (await locatorCount3(createButton) !== 1) {
  ...
  if (await locatorCount3(nameInput) !== 1) {
  ...
  if (await locatorCount3(appearanceButton) !== 1) {
  ...
  if (await locatorCount3(color) !== 1 || await locatorCount3(icon) !== 1) {
  ...
  if (await locatorCount3(submit) !== 1) {
...
async function waitForLocator(locator, page, timeoutMs) {
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    if (await locatorCount3(locator) > 0) return true;
    await page.waitForTimeout?.(100);
  }
  return false;
}
...
function projectHandleFromUrl(value) {
  try {
    const parsed = new URL(value, CHATGPT_HOME2);
    if (parsed.protocol !== "https:" || parsed.hostname !== "chatgpt.com") return void 0;
    const segments = parsed.pathname.split("/").filter(Boolean);
    const gIndex = segments.indexOf("g");
...
function generationStateFromHtml(html) {
  const normalize = (value) => value.replace(/<[^>]*>/g, " ").replace(/&nbsp;/gi, " ").replace(/\s+/g, " ").trim().toLowerCase();
